...
aiRouter.post('/summarize/:chatId', async (req: AuthRequest, res) => {
  ...
  const transcript = messages.reverse().map((m: any) => `${m.sender.name}: ${m.content}`).join('\n');
  ...
  try {
    ...
    const summaryMsg = await prisma.message.create({
      data: { chatId, senderId: req.userId!, content: `📝 Resumen: ${summary}`, contentType: 'SYSTEM' }
    });
    io.to(chatId).emit('new_message', summaryMsg);
