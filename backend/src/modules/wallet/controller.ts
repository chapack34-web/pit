...
walletRouter.post('/transfer', async (req: AuthRequest, res) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx: any) => {
      const sender = await tx.wallet.findUnique({ where: { userId: req.userId! } });
      if (!sender || sender.balance < amount) throw new Error('Saldo insuficiente');

      await tx.wallet.update({ where: { userId: req.userId! }, data: { balance: { decrement: amount } } });
